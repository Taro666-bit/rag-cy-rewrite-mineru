...
class VectorDBIngestor:
    ...
    @retry(wait=wait_exponential(multiplier=2, min=5, max=60), stop=stop_after_attempt(4))
    def _get_embeddings(self, text: Union[str, List[str]], model: str = "text-embedding-v4") -> List[float]:
        ...
        for i in range(0, len(text_chunks), MAX_BATCH_SIZE):
            ...
            # with open(LOG_FILE, 'a', encoding='utf-8') as f:
            #     f.write('resp='+str(resp)+'\n')
            if 'output' in resp and resp['output'] is not None and 'embeddings' in resp['output']:
            ...
            elif 'output' in resp and resp['output'] is not None and 'embedding' in resp['output']:
                ...
                reason = 'output为None' if ('output' in resp and resp['output'] is None) else 'output缺少embedding字段'
                print(f'[WARN] API返回异常: {reason}, 完整resp={resp}')
                with open(LOG_FILE, 'a', encoding='utf-8') as f:
                    f.write(f'API返回异常: {reason}\nresp={resp}\n{"-"*60}\n')
                raise RuntimeError(f"DashScope embedding API返回格式异常 ({reason}): {resp}")
            time.sleep(3)  # 批次间延迟，避免触发 QPS 限流/连接重置
        return embeddings
