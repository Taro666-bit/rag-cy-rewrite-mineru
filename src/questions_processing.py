...
class QuestionsProcessor:
    def __init__(
        ...
        answering_model: str = "glm-5", # gpt-4o-2024-08-06
        reranking_model: str = "gte-rerank-v2",
        full_context: bool = False,
        use_neighbor_chunks: bool = False,  # 将每个检索到的chunk扩展为其±1邻居
        use_stream: bool = True             # DashScope是否启用流式输出
    ):
        ...
        self.use_neighbor_chunks = use_neighbor_chunks
    ...
    @staticmethod
    def _clean_chunk_display_text(text: str) -> str:
        """清理检索摘要中的脏文本（HTML标签、页眉页脚、多余空白等）"""
        # 1. 去掉 HTML 标签
        text = re.sub(r'<[^>]*>', '', text)
        # 2. 去掉残留的 HTML 属性片段（如 colspan="1" rowspan="1">）
        text = re.sub(r'\b(col|row)span\s*=\s*"[^"]*"\s*', '', text)
        # 3. 去掉形如 "数字/数字" 的页码尾缀（如 "121/222"）
        text = re.sub(r'\n?\s*\d+/\d+\s*$', '', text)
        # 4. 压缩多余空白（多个换行→最多两个换行，多个空格→一个空格）
        text = re.sub(r'\n{3,}', '\n\n', text)
    ...
    def get_answer_for_company(self, company_name: str, question: str, schema: str) -> dict:
        ...
        if self.full_context:
            ...
            retrieval_results = retriever.retrieve_by_company_name(
                ...
                use_neighbor_chunks=self.use_neighbor_chunks
            )
            t3 = time.time()
        if not retrieval_results:
        ...
        # 记录检索统计，供外部展示
        unique_pages = len(set(r["page"] for r in retrieval_results))
        self.last_retrieval_info = {
            "total_chunks": len(retrieval_results),
            "unique_pages": unique_pages,
            "top_n": self.top_n_retrieval,
            "neighbor_expansion": self.use_neighbor_chunks
        }
        ...
        t6 = time.time()
        print(f"  [检索] 返回{len(retrieval_results)}条 | 初始化{t1-t0:.1f}s | 检索{t3-t1:.1f}s | 构建上下文{t5-t4:.1f}s")
        print(f"  [LLM] 调用耗时: {t6-t5:.1f}s")
    ...
    def get_answer_for_company_stream(self, company_name: str, question: str, schema: str):
        """
        流式版本：先检索，再实时 yield LLM token，最后 yield 结构化答案。
        ...
        # 先 yield 检索结果，让前端立即显示（不等待 LLM）
        top_chunks = retrieval_results[:3]
        retrieval_summary_parts = []
        for i, r in enumerate(top_chunks):
            page = r['page']
            raw_text = r['text'][:500]
            text = self._clean_chunk_display_text(raw_text)
            if len(raw_text) > 500:
                text += "..."
            retrieval_summary_parts.append(f"📄 第 {page} 页（相关度 #{i+1}）:\n{text}")
        retrieval_summary = "\n\n---\n\n".join(retrieval_summary_parts)
        yield {
            "type": "retrieval",
            "rag_context": rag_context,
            "retrieval_info": self.last_retrieval_info,
            "retrieval_summary": retrieval_summary
        }
        
        t5 = time.time()
        stream_gen = self.openai_processor.get_answer_from_rag_context_stream(
            question=question,
            rag_context=rag_context,
            schema=schema,
            model=self.answering_model
        ...
        if self.new_challenge_pipeline and answer_dict:
            pages = answer_dict.get("relevant_pages", [])
            validated_pages = self._validate_page_references(pages, retrieval_results)
            answer_dict["relevant_pages"] = validated_pages
            answer_dict["references"] = self._extract_references(validated_pages, company_name)
        
        yield {"type": "done", "answer": answer_dict, "retrieval_info": self.last_retrieval_info}
    ...
    def _post_process_submission_answers(self, processed_questions: List[dict]) -> List[dict]:
        ...
        for q in processed_questions:
            ...
            if value == "N/A":
                ...
                # 页码从 1-based 转为 0-based（调试时使用 1-based 更直观）
                references = [
                    {
    ...
    def process_single_question(self, question: str, kind: str = "string"):
        ...
        t0 = time.time()
        print(f"\n{'='*60}")
        print(f"  [查询] {question[:80]}{'...' if len(question) > 80 else ''}")
        print(f"{'='*60}")
        ...
        if self.new_challenge_pipeline:
            ...
            extracted_companies = re.findall(r'"([^"]*)"', question)
        t1 = time.time()
        print(f"  [抽取] 公司名: {extracted_companies} | {t1-t0:.2f}s")
        if len(extracted_companies) == 0:
            raise ValueError("No company name found in the question.")
        if len(extracted_companies) == 1:
            company_name = extracted_companies[0]
    ...
    def process_single_question_stream(self, question: str, kind: str = "string"):
        ...
        if len(extracted_companies) != 1:
            ...
            print(f"  [多公司] 耗时: {t3-t2:.2f}s")
            print(f"  [总耗时] {t3-t0:.2f}s")
            yield {"type": "done", "answer": answer_dict, "retrieval_info": self.last_retrieval_info if hasattr(self, 'last_retrieval_info') else None}
            return
        
        company_name = extracted_companies[0]
        t2 = time.time()
        stream_gen = self.get_answer_for_company_stream(
            company_name=company_name,
            question=question,
            schema=kind
        )
        ...
        for item in stream_gen:
            if item["type"] == "token":
                ...
                final_retrieval_info = item.get("retrieval_info")
        
        t3 = time.time()
        print(f"  [检索+推理] 总耗时: {t3-t2:.2f}s")
        print(f"  [总耗时] {t3-t0:.2f}s")
        yield {"type": "done", "answer": final_answer, "retrieval_info": final_retrieval_info}
